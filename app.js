...
/* ===== Cloud Sync (Firebase Firestore) =====
   - إذا لم تضع إعدادات Firebase الحقيقية → يبقى التطبيق محلي (LocalStorage).
   - إذا وضعت الإعدادات → يصير مزامنة بين كل الأجهزة على نفس المشروع.
*/

// ضع إعدادات Firebase هنا (قيم وهمية حالياً)
...
// مسار الوثيقة التي نخزن بها كل البيانات (مشاركة لكل الأجهزة)
const CLOUD_DOC_PATH = { col: "accounts", doc: "shared-v1" };
...
function cloudDocRef(){
  return cloudDb.collection(CLOUD_DOC_PATH.col).doc(CLOUD_DOC_PATH.doc);
}

async function cloudLoadOnce(){
  if(!cloudEnabled) return null;
  const snap = await cloudDocRef().get();
  if(!snap.exists) return {clients:[], invoices:[], expenses:[]};
...
function cloudListen(){
  ...
  cloudUnsub = cloudDocRef().onSnapshot((snap)=>{
    ...
    if(!remote) return;
    // لا تكتب فوق التغييرات المحلية أثناء كتابة جارية
    const remoteStr = JSON.stringify(remote);
    const localStr = JSON.stringify(data);
    if(remoteStr === localStr) return;
    cloudApplyingRemote = true;
    data = remote;
    // حافظ على نسخة محلية ككاش (مفيد للأوفلاين)
    try{ localStorage.setItem(LS_DATA, JSON.stringify(data)); }catch{}
    renderAll();
    cloudApplyingRemote = false;
    toast('✅ تم تحديث البيانات من السحابة');
  }, (err)=>{
    console.warn("cloudListen error", err);
  });
}

function cloudQueueWrite(){
  if(!cloudEnabled) return;
  ...
  cloudWriteTimer = setTimeout(async ()=>{
    try{
      ...
      toast('⚠️ تعذّر الحفظ على السحابة (تحقق من الإعدادات/النت)');
    }
  }, 450); // debounce
}

// واجهة موحدة للحفظ: سحابة + كاش محلي
function persistData(){
  try{ localStorage.setItem(LS_DATA, JSON.stringify(data)); }catch{}
  cloudQueueWrite();
}

async function cloudStartAfterLogin(){
  // فعّل السحابة إذا كانت الإعدادات صحيحة
  if(!initCloud()){
    toast('ℹ️ وضع محلي: ضع إعدادات Firebase لتفعيل المزامنة');
    return;
  }
  // اجلب آخر نسخة من السحابة ثم استمع للتغييرات
  try{
    const remote = await cloudLoadOnce();
    if(remote){
      data = remote;
      try{ localStorage.setItem(LS_DATA, JSON.stringify(data)); }catch{}
      renderAll();
    ...
    console.warn("cloudStart error", e);
    toast('⚠️ فشل الاتصال بالسحابة');
  }
}
...
function localSaveData(){ localStorage.setItem(LS_DATA, JSON.stringify(data)); }
...
function editClient(id){
  ...
  takeSnapshot();
  c.name=v;
  persistData(); renderAll(); toast('تم التعديل ✏️');
}
...
function editInvoice(id){
  ...
  persistData(); renderAll(); toast('تم تعديل الفاتورة ✏️');
}
function deleteInvoice(id){
  if(!confirm('حذف الفاتورة؟')) return;
  takeSnapshot();
  data.invoices=data.invoices.filter(x=>x.id!==id);
  persistData(); renderAll(); toast('تم حذف الفاتورة 🗑');
}
...
function editExpense(id){
  ...
  persistData(); renderAll(); toast('تم تعديل المصروف ✏️');
}
function deleteExpense(id){
  if(!confirm('حذف المصروف؟')) return;
  takeSnapshot();
  data.expenses=data.expenses.filter(x=>x.id!==id);
  persistData(); renderAll(); toast('تم حذف المصروف 🗑');
}
